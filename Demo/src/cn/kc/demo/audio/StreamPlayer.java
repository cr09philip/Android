...
public class StreamPlayer {
	...
	public StreamPlayer(int frequency, int channel, int sampbit) {
...
//			Log.i(TAG, "catch exception...");
	...
	public void playFiles(String path,String name){
		/*
		mPlayer = new AudioPlayer(8000, AudioFormat.CHANNEL_OUT_MONO, AudioFormat.ENCODING_PCM_16BIT);
		File file = new File(mContext.mAppPath+"/sh.pcm");
		BufferedInputStream input = new BufferedInputStream(new FileInputStream(file));
		
		byte[] buffer = new byte[65536];
		int byteOffset = 0;
		input.read(buffer, byteOffset, 65536);
		int nLength = mPlayer.getPrimePlaySize();
		
		while(byteOffset < 65536){
			mPlayer.playAudioTrack(buffer, byteOffset, nLength/2);
			byteOffset += nLength/2;
		}
		*/
	}
